...
const TimePickerInput = ({
  ...
  // const [isNow, setIsNow] = useState(new Date());
  ...
  const now = new Date();
  const isToday = storedDate?.date
    ? new Date(storedDate.date).toDateString() === now.toDateString()
    : false;
  ...
  const handleManualHourChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value !== "") {
      ...
      // setSelectedHour(h);
      if (h >= 0 && h <= 23) {
        // if (isToday && h < currentHour) return; // ← ضيف السطر ده
                  ...
                  {hours.map((hour) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isPast}
                        className={cn(
                          "w-16 text-sm py-2 rounded transition-colors",
                          isPast
